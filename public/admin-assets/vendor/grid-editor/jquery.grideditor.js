...
$.fn.gridEditor = function( options ) {
    ...
    self.each(function(baseIndex, baseElem) {
        ...
        if (baseElem.children().length && !baseElem.find('.wrapper').length) {
            var children = baseElem.children();
            var newRow = $('<div class="wrapper"></div>').appendTo(baseElem);
            newRow.find('.wrapper').append(children);
        }
        ...
        var settings = $.extend({
            ...
            'imageDir'          : '',
            'beforeUpload'      : function(){console.log('uploading...')},
            'afterUpload'       : function(){console.log('file uploaded')},
        }, options);
        ...
        function setup() {
            ...
            $.each(settings.new_row_layouts, function(j, layout) {
                var btn = $('<a class="btn btn-xs btn-primary" />')
                    .attr('title', 'Ajouter colonne ' + layout.join('-'))
                    .on('click', function() {
                        var section = createSection().appendTo(canvas);
                        var row = createRow().appendTo($(section).find('.container'));
            ...
            layoutDropdown = $('<div class="dropdown pull-right ge-layout-mode">' +
                ...
                '<ul class="dropdown-menu" role="menu">' +
                    '<li style="display:none" data-layout="desktop"><a data-width="auto" title="Desktop"><span>Desktop</span></a></li>' +
                    '<li style="display:none" data-layout="tab"><a title="Tablette"><span>Tablette</span></li>' +
                    '<li style="display:none" data-layout="phone"><a title="Mobile"><span>Mobile</span></a></li>' +
                    '</ul>' +
                '</div>');
            // Buttons on right
            if(settings.layouts){
                settings.layouts.forEach(function(obj, i) {
                   layoutDropdown.find('li[data-layout="'+obj+'"]').show();
                });
            }
            ...
            var htmlButton = $('<button title="Edit Source Code" type="button" class="btn btn-xs btn-primary gm-edit-mode"><span class="glyphicon glyphicon-chevron-left"></span><span class="glyphicon glyphicon-chevron-right"></span></button>')
                ...
                .appendTo(btnGroup)
            ;
            var previewButton = $('<button title="Prévisualiser" type="button" class="btn btn-xs btn-primary gm-preview"><span class="glyphicon glyphicon-eye-open"></span></button>')
        ...
        function init() {
            ...
            layoutDropdown.find('li[data-layout="'+settings.layouts[0]+'"] a').click();
        ...
        function createRowControls() {
            canvas.find('.row').each(function() {
                ...
                createTool(drawer, 'Déplacer', 'ge-move', 'glyphicon-move');
                createTool(drawer, 'Configuration', '', 'glyphicon-cog', function() {
                    details.toggle();
                });
                settings.row_tools.forEach(function(t) {
                    createTool(drawer, t.title || '', t.className || '', t.iconClass || 'glyphicon-wrench', t.on);
                });
                createTool(drawer, 'Supprimer ligne', '', 'glyphicon-trash', function() {
                    row.slideUp(function() {
                        row.remove();
                    });
                });
                createTool(drawer, 'Ajouter colonne', 'ge-add-column', 'glyphicon-plus-sign', function() {
        ...
        function createColControls() {
            canvas.find('.column').each(function() {
                ...
                createTool(drawer, 'Reduire\n(Maintenir Maj pour le minimum)', 'ge-decrease-col-width', 'glyphicon-minus', function(e) {
                ...
                createTool(drawer, 'Agrandir\n(Maintenir Maj pour le maximum)', 'ge-increase-col-width', 'glyphicon-plus', function(e) {
                    var colSizes = settings.valid_col_sizes;
                    var curColClass = colClasses[curColClassIndex];
                    var curColSizeIndex = colSizes.indexOf(getColSize(col, curColClass));
                ...
                createTool(drawer, 'Supprimer colonne', '', 'glyphicon-trash', function() {
                    col.animate({
                        opacity: 'hide',
                        width: 'hide',
                        height: 'hide'
                ...
                createTool(drawer, 'Ajouter ligne', 'ge-add-row', 'glyphicon-plus-sign', function() {
                    var row = createRow();
                    col.append(row);
        ...
        function createSection(){
            return $('<section><div class="container"/></section>');
        }
        ...
        init: function(settings, contentAreas) {
            ...
            contentAreas.each(function() {
                ...
                if (!contentArea.hasClass('active')) {
                    ...
                    var sendFile = function(files,editor) {
                            ...
                            $.ajax({
                                    url: settings.imageUploadUrl,//Upload url
                                    data: data,
                                    cache: false,
                                    contentType: false,
                                    processData: false,
                                    ...
                                    success: function(data){
                                        var $file;
                                            for (var i = 0; i < data.length; i++) {
                                                editor.summernote("insertImage", settings.imageDir+'/'+data[i].file,'filename');
                                            }
                                        settings.afterUpload();
                                    },
